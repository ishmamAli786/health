			...
			<td>'.$rec_total.'</td>
				...
				foreach($dan as $abc){
					...
					foreach ($abc as $dat => $pay ){
						...
						//find pending installment
							...
							if(($feesum > $rec_total)){
									...
									if($dat >= (date("Y-m-d"))){
										$status = true;
										$state =  "<br>".$dat."----this installment ".($feesum-$rec_total);
									}else {
										//find next pending installment
										$state =  "<br>".$dat."----Processing more pending Installments";
									}
										
										//if fee clear than echo next fee date and amount
								}
								elseif($feesum == $rec_total and $dat >= (date("Y-m-d"))){
										$state =  "Fee clear. Payable Fee till <b>".$dat."</b> is <b>".$feesum."</b> <br>received ".$rec_total;
											$status = true;
									}
					}
					...
					if( ($dat < (date("Y-m-d"))) AND $feesum > $rec_total){
							$state =  "<br>".$dat."Installments date passed but but fee Pending ".($feesum-$rec_total);;
						}else{
							//echo "";
						}
	}
