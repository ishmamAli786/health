	...
	<div class="container-fluid">
		...
		<?php if(isset($_GET['status']) && isset($_GET['app_code'])){
				...
				if(isset($_POST['app_date'])){
					...
					$sql = mysqli_query($db, "UPDATE appointments SET REMARKS = '$remarks', STATUS = 2 WHERE CODE = '$app_code'");
					header("Location:appointments.php");
				}
		?>
		<style>
		...
		<div class="col-md-9" style="padding-top:20%;">
			...
			<form action="appointments.php?status=<?=$_GET['status'];?>&app_code=<?=$_GET['app_code'];?>" method="post">
			  <div class="form-row">
				...
				<div class="form-group col-md-12">
				  <label for="app_date">REMARKS ABOUT Report</label>
		...
		<div class="col-md-9" style="padding-top:20%;">
			...
			<h2 style="color:#000;clear:both;">Reports</h2>
			...
			<table class="table table-bordered">
				...
				<tbody>
					<?php
						$user_code = $_SESSION['code'];
						$SQuery = mysqli_query($db, "SELECT * FROM `app_view` WHERE DOCTORS_CODE = '$user_code' AND APP_STATUS IN (0, 1, 2) ORDER BY APPOINT_DATE");
					...
					?>
						<tr>
							...
							<td>
								...
								<button class="btn btn-info">No Report</button>
								<?php }else{ ?>
